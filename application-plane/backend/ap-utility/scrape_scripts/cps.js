// cps client scrape script (generated by login-flow-recorder).
//
...
var EXTRACTION = () => {
  ...
  // Most recent = row with latest billing date
  let best = null, bestTime = -Infinity;
  rows.forEach(tr => {
    const c = tr.querySelectorAll('td');
    if (c.length < 3) return;
    const billIso = parseDate(c[0].textContent);
    const dueIso = parseDate(c[1].textContent);
    const total = parseAmt(c[2].textContent);
    const t = billIso ? new Date(billIso).getTime() : -Infinity;
    if (t > bestTime) { bestTime = t; best = { due_date: dueIso, total }; }
  });
  return { bills: best ? [best] : [], usage: [] };
};
